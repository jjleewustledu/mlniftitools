...
function [img,hdr] = read_image(hdr,filetype,fileprefix,machine,img_idx,dim5_idx,dim6_idx,dim7_idx,old_RGB,slice_idx)
   ...
   elseif hdr.dime.datatype == 511 & hdr.dime.bitpix == 96
      ...
      % remove squeeze
      avail     = numel(img)/img_siz; %%%%% JJL, 2011oct30, for sagittal MPRAGE from Siemens Trio VB17+
                                      %%%%% overlaid annotation occupies an extra slice
      slice_siz = prod(hdr.dime.dim(2:3));
      if 1 == avail
          img = (reshape(img, [hdr.dime.dim(2:3) length(slice_idx) length(img_idx) length(dim5_idx) length(dim6_idx) length(dim7_idx)]));
      else
          try
              img0 = img;
              img  = (reshape(img, [hdr.dime.dim(2:3) numel(img)/slice_siz length(dim5_idx) length(dim6_idx) length(dim7_idx)]));
          catch ME
              img  = img0;
              handwarning(ME, 'problem in load_untouch_nii_img.m with reshaping img');
          end
      end
   end
